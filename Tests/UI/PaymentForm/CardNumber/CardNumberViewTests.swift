...
final class CardNumberViewTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        guard let mockViewModel = mockViewModel else {
            XCTFail("mockViewModel is nil")
            return
        }
    ...
    func test_textFieldShouldReturn_returnTrue() {
        XCTAssertTrue(subject?.textFieldShouldReturn() == true)
    ...
    func test_textField_rangeIsInvalid_returnTrue() {
        ...
        // then
        XCTAssertTrue(subject?.textField(textField, shouldChangeCharactersIn: range, replacementString: "") == true)
        XCTAssertNil(mockViewModel?.eagerValidateCalledWith)
        XCTAssertEqual(subject?.schemeIcon, .blank)
    }
    ...
    func test_textField_noUpdateFromViewModel() {
        ...
        // when
        XCTAssertFalse(subject?.textField(textField, shouldChangeCharactersIn: NSRange(), replacementString: "12345") == true)
    ...
    func test_textFieldShouldEndEditing_success() {
        ...
        // when
        XCTAssertTrue(subject?.textFieldShouldEndEditing(textField: UITextField(), replacementString: "12345") == true)

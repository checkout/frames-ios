...
class BillingFormSummaryView: UIView {
    ...
    func update(style: BillingSummaryViewStyle) {
        ...
        update(state: StateUpdate(
            titleLabelUpdate: nil,
            hintLabelUpdate: nil,
            summaryLabelUpdate: nil,
            imageContainerViewUpdate: ImageContainerView.StateUpdate(
                image: self.style?.button.image,
                tintColor: style.button.imageTintColor,
                isHidden: false,
                animated: false 
            )
        ))
...
extension BillingFormSummaryView {
    ...
    private func setupButton() {
        ...
        NSLayoutConstraint.activate([
            imageContainerView.topAnchor.constraint(equalTo: summarySeparatorLineView.bottomAnchor, constant: 10),
...
extension BillingFormSummaryView: Stateful {
    struct StateUpdate {
        let titleLabelUpdate: LabelView.StateUpdate?
        let hintLabelUpdate: LabelView.StateUpdate?
        let summaryLabelUpdate: LabelView.StateUpdate?
        let imageContainerViewUpdate: ImageContainerView.StateUpdate?
    }

    func update(state update: StateUpdate) {
        if let titleLabelUpdate = update.titleLabelUpdate {
            titleLabel.update(state: titleLabelUpdate)
        }

        if let hintLabelUpdate = update.hintLabelUpdate {
            hintLabel.update(state: hintLabelUpdate)
        }

        if let summaryLabelUpdate = update.summaryLabelUpdate {
            summaryLabel.update(state: summaryLabelUpdate)
        }

        if let imageContainerViewUpdate = update.imageContainerViewUpdate {
